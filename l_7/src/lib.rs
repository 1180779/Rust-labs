...
pub fn sum_squares_odd(list: &[u32]) -> u32 {
    ...
    list.iter().filter(|x| *x & 1 == 1).map(|x| x * x).sum()
}
...
pub fn cycles_2(edges: &[(u32, u32)]) -> Vec<u32> {
    ...
    edges
        ...
        .filter(|&(&p1, &p2)| p1.0 == p2.1 && p1.1 == p2.0 && p1.0 != p2.0)
        .flat_map(|(&p1, &_p2)| [p1.0, p1.1])
        .sorted()
        .unique()
        .collect()
}
...
pub fn primes(n: u32) -> Vec<u32> {
    ...
    (3..n).step_by(2).fold(vec![2], |mut primes, i| {
        let is_prime = primes
            .iter()
            .take_while(|&&p| p * p <= i)
            .all(|&p| i % p != 0);
...
pub fn run_length_encode(list: &[u32]) -> Vec<(u32, usize)> {
    list.iter()
        ...
        .chunk_by(|x| *x)
        .into_iter()
        .map(|(value, group)| (value, group.count()))
        .collect()
}
...
pub fn compose_all(fns: &[fn(i32) -> i32]) -> impl Fn(i32) -> i32 {
    move |x| fns.iter().copied().fold(x, |acc, f| f(acc))
}
